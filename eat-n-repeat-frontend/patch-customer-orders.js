...
for (let i = 0; i < lines.length; i++) {
    ...
    if (inOrdersBlock) {
        // Count braces/parentheses to find the end
        // A simpler way: we know it ends right before {/* TAB 3: MENU MANAGEMENT */}
        if (lines[i].includes('{/* TAB 3: MENU MANAGEMENT */}')) {
            endIndex = i;
            break;
        }
    }
            ...
            <div className="space-y-6">
                            ...
                            {filteredActive.map((order) => (
                              <tr key={order.id} className="border-b border-accent/5 hover:bg-accent-light/10">
                                ...
                                <td className="px-4 py-3">
                                  <span className="inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold uppercase text-gray-600 border border-gray-200">
                                ...
                                <td className="px-4 py-3">
                                  {order.paid ? (
                                    <span className="inline-flex rounded-full bg-green-50 px-2 py-0.5 text-xs font-semibold text-green-700 border border-green-200">Confirmed Paid</span>
                                  ) : (
                                    <span className="inline-flex rounded-full bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-700 border border-amber-200">Pending</span>
                                  )}
                                </td>
                                <td className="px-4 py-3">
                                  <AdminSelect
                                    ...
                                    className="!py-1 !text-xs max-w-32"
                                  >
                                    <option value="pending">Pending</option>
                                    <option value="confirmed">Confirmed</option>
                                    <option value="preparing">Preparing</option>
                          ...
                          <div key={order.id} className="bg-white rounded-xl border border-accent/10 p-4 shadow-sm flex flex-col gap-3">
                            ...
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                {order.paid ? (
                                  <span className="text-xs font-bold text-green-700">Confirmed Paid</span>
                                ) : (
                            ...
                            <button
                              onClick={() => setSelectedOrderDetails(order)}
                              className="w-full mt-1 py-2 bg-accent-light/50 text-accent font-bold text-sm rounded-lg border border-accent/10 hover:bg-accent-light transition-colors"
                            >
                              View Details
                            </button>
                          </div>
              ...
              <AdminPanel title="Customer Order History" subtitle="Fulfilled or cancelled records">
                ...
                <div className="p-2 overflow-x-auto">
                  {filteredHistory.length === 0 ? (
                    <div className="text-center py-8 text-muted">No history found.</div>
                  ) : (
                    <table className="w-full text-left text-sm min-w-[640px]">
                      ...
                      <tbody>
                        {filteredHistory.map((order) => (
                          <tr key={order.id} className="border-b border-accent/5 hover:bg-accent-light/10 text-muted">
                            <td className="px-4 py-3 font-bold text-[#2B2523]">{order.orderId}</td>
                            <td className="px-4 py-3">{order.customerName || "Walk-in"}</td>
                            <td className="px-4 py-3 text-xs">{order.time}</td>
                            <td className="px-4 py-3 text-xs capitalize">{order.orderType || "dine-in"}</td>
                            <td className="px-4 py-3 font-semibold">₱{order.total?.toFixed(2) || (order.subtotal + (order.deliveryFee||0)).toFixed(2)}</td>
                            <td className="px-4 py-3">
                              <span className={\`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold \${order.status === 'completed' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'} border\`}>
                                {order.status}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-right">
              ...
              {selectedOrderDetails && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in">
                  <div className="bg-[#FFF8F0] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                    {/* Header */}
                    <div className="flex justify-between items-center p-5 border-b border-accent/10 bg-white">
                      <div>
                        ...
                        <span className="inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold uppercase text-gray-600 mt-1">
                    ...
                    <div className="p-5 overflow-y-auto space-y-6">
                      ...
                      {/* Customer Info */}
                      <div className="bg-white p-4 rounded-xl shadow-sm border border-accent/5">
                        <h3 className="text-xs font-bold uppercase text-muted tracking-wider mb-3">Customer Details</h3>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <p className="text-muted text-xs">Name</p>
                            <p className="font-medium">{selectedOrderDetails.customerName || "Walk-in Customer"}</p>
                          </div>
                          ...
                          <div>
                            ...
                            <p className="font-medium">{selectedOrderDetails.time}</p>
                          </div>
                          <div>
                            <p className="text-muted text-xs">Status</p>
                            <p className="font-bold text-accent capitalize">{selectedOrderDetails.status}</p>
                          </div>
                        </div>
                        {selectedOrderDetails.orderType === 'delivery' && selectedOrderDetails.address && (
                          <div className="mt-4 pt-4 border-t border-accent/5">
                            <p className="text-muted text-xs mb-1 flex items-center gap-1"><MapPin className="h-3 w-3" /> Delivery Address</p>
                            <p className="font-medium text-sm">{selectedOrderDetails.address}</p>
                            <button
                              onClick={() => handleOpenChat(selectedOrderDetails.customerName, selectedOrderDetails.orderId)}
                              className="mt-3 flex items-center justify-center gap-2 w-full py-2 bg-accent/5 text-accent font-bold text-sm rounded-lg hover:bg-accent/10 transition-colors"
                            >
                      ...
                      {/* Order Items */}
                      <div>
                        <h3 className="text-xs font-bold uppercase text-muted tracking-wider mb-3 px-1">Order Items</h3>
                        <div className="bg-white rounded-xl shadow-sm border border-accent/5 overflow-hidden">
                          <div className="p-4 space-y-3">
                            {/* Assuming items is a string for mock data, normally it's an array. We'll just display it. */}
                            <div className="flex justify-between items-start text-sm">
                              <span className="font-medium">{selectedOrderDetails.items}</span>
                            </div>
                          </div>
                          
                          <div className="bg-accent-light/30 p-4 border-t border-accent/5 space-y-2 text-sm">
                            ...
                            <div className="flex justify-between font-bold text-lg text-[#2B2523] pt-1">
                              <span>Total</span>
                              <span>₱{selectedOrderDetails.total?.toFixed(2) || (selectedOrderDetails.subtotal + (selectedOrderDetails.deliveryFee||0)).toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Payment Status */}
                      <div className="flex justify-between items-center p-4 bg-white rounded-xl shadow-sm border border-accent/5">
                        <span className="text-sm font-bold text-[#2B2523]">Payment Status</span>
                        {selectedOrderDetails.paid ? (
                          <span className="px-3 py-1 bg-green-100 text-green-800 font-bold text-xs rounded-full border border-green-200">Confirmed Paid</span>
                        ) : (
                          <span className="px-3 py-1 bg-amber-100 text-amber-800 font-bold text-xs rounded-full border border-amber-200">Pending</span>
                        )}
                      </div>
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    console.log("Failed to find Orders block boundaries.");
}
